...
int*** memory();/*Выделяем память под трёхмерный массив*/
int** memory1();/*Выделяем память под двумерный массив*/
char* txt(int*** A, char my[N]);/*Считываем из файла значения и заполняем 3 массива*/ //
...
void main()
{
	...
	int*** A1 = memory();//Выделение памяти под трёхмерные массивы
	int*** A2 = memory();
	int*** A3 = memory();
	int*** A = memory();

	int** och_win = memory1();//Выделение памяти под двумерные массивы
	int** och_loss = memory1();
...
int** memory1()
{
	...
	for (int i = 0; i < N; i++) {
		och[i] = (int*)malloc(N * sizeof(int));
	}
	return och;
}

char* txt(int*** A, char my[N])
{
	FILE* pog;
